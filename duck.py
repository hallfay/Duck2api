...
class XVqdManager:
    ...
    async def fetch_new_x_vqd_4(self):
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.get("https://duckduckgo.com/duckchat/v1/status",
    ...
    async def generate():
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(999, read=250), follow_redirects=True) as stream_client:
                ...
                async with stream_client.stream('POST', 'https://duckduckgo.com/duckchat/v1/chat',
...
x_vqd_manager = XVqdManager(update_interval=10)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=5179)